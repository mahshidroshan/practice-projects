...
let answerRevealed = false; // Flag to track if the answer is revealed
let parentDiv = document.querySelector('.content');

function clearAll() {
    const contentItems = document.querySelectorAll('.content img, .content div');

    contentItems.forEach((item) => item.style.display = 'none');
    currentRiddle = null;
    answerRevealed = false; // Reset answerRevealed flag
}
...
function showJoke() {
  ...
  parentDiv.insertBefore(joke, p2);
}
...
function showQuote() {
  ...
  currentQuote = getRandomData('quotes');
  ...
  parentDiv.insertBefore(quote, p3); 
  parentDiv.insertBefore(author, quote.nextElementSibling);
}
...
function showRiddle() {
    ...
    // Retrieve a random riddle and store it along with its answer
    currentRiddle = getRandomData('riddles');

    const riddleDiv = document.createElement('div');
    riddleDiv.textContent = currentRiddle.question;
    parentDiv.appendChild(riddleDiv);
}

function revealAnswers() {
    // Check if a riddle is currently displayed
    if (!currentRiddle) {
        alert("No riddle is currently displayed.");
        return;
    }
    ...
    // Create a div for the riddle answer and append it to the parent div
    const answerText = "Answer: " + currentRiddle.answer;
    const newAnswerDiv = document.createElement('div');
    newAnswerDiv.textContent = answerText;
    parentDiv.appendChild(newAnswerDiv);
    newAnswerDiv.style.marginTop = "10px";

    answerRevealed = true; // Set answerRevealed flag to true
}
